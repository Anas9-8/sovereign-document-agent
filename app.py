...
st.set_page_config(
    page_title="Sovereign Document Agent",
    page_icon="🛡️",
    layout="wide"
)
...
tab1, tab2, tab3 = st.tabs(["📄 Dokument verarbeiten", "📊 Ergebnisse", "🛡️ Governance"])
        ...
        if st.button("▶️ Dokument verarbeiten", type="primary"):
            with st.spinner("Dokument wird verarbeitet..."):
                ...
                if is_sensitive:
                    ...
                    st.info("Dieses Dokument kann sensible Informationen enthalten. DSGVO erfordert ausdrückliche Zustimmung.")
                    ...
                    with col1:
                        if st.button("✅ Verarbeitung genehmigen", key="approve"):
                            ...
                            governance.audit.log_event('approval_decision', {'decision': 'approved'}, 'success')
                        else:
                            st.stop()
                    ...
                    with col2:
                        if st.button("❌ Verarbeitung ablehnen", key="deny"):
                            ...
                            governance.audit.log_event('approval_decision', {'decision': 'denied'}, 'success')
                            os.remove(temp_path)
                            st.stop()
                else:
...
{text[:500]}"""
                ...
                try:
                    ...
                    try:
                        ...
                        st.warning("⚠️ JSON-Parsing fehlgeschlagen, verwende manuelle Extraktion...")
                        ...
                        data['invoice_date'] = date_match.group(1) if date_match else "N/A"
                        
                        cust_match = re.search(r'"customer_name"\s*:\s*"([^"]*)"', json_str)
                        data['customer_name'] = cust_match.group(1) if cust_match else "N/A"
                        
                        amt_match = re.search(r'"total_amount"\s*:\s*"([^"]*)"', json_str)
                        data['total_amount'] = amt_match.group(1) if amt_match else "N/A"
                    ...
                    # Log
                    governance.log_ai_extraction('llama3.2:1b', success=True, fields_extracted=len(data))
                    governance.log_processing_end(uploaded_file.name, success=True, extracted_fields=len(data))
                    ...
                    with col1:
                        st.metric("📄 Rechnungsnummer", data.get('invoice_number', 'N/A'))
