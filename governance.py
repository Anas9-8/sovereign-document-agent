...
def check_file_guardrails(file_path):
    ...
    if not os.path.exists(file_path):
        ...
        logger.error(msg)
        audit.log_event(
            "guardrail_check", {"file": file_path, "reason": "not_found"}, "failure"
        )
        return False, msg
    ...
    if file_ext not in GUARDRAILS["allowed_extensions"]:
        ...
        audit.log_event(
            "guardrail_check",
            {"file": file_path, "reason": "invalid_extension"},
            "failure",
        )
        return False, msg
    ...
    if file_size_mb > max_size:
        ...
        audit.log_event(
            "guardrail_check",
            {"file": file_path, "size_mb": file_size_mb, "reason": "too_large"},
            "failure",
        )
        return False, msg
    ...
    audit.log_event(
        "guardrail_check", {"file": file_path, "size_mb": file_size_mb}, "success"
    )
    return True, "OK"
...
def request_human_approval(reason):
    ...
    while True:
        ...
        if response in ["yes", "y"]:
            ...
            audit.log_event(
                "approval_decision", {"decision": "denied", "reason": reason}, "success"
            )
            return False
        else:
...
def log_ai_extraction(model, success=True, fields_extracted=0):
    ...
    audit.log_event(
        "ai_extraction", {"model": model, "fields_extracted": fields_extracted}, status
    )
...
def print_governance_report():
    ...
    print(
        f"   • Sensitive keywords: {len(GUARDRAILS['require_human_approval_keywords'])}"
    )
